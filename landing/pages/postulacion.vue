	...
	<div v-if="!loading" style="height: 100vh">
		<div class="ma-4 d-flex justify-space-between align-center">
			<a id="logo-appbar" tabindex="0" href="https://hablaqui.cl/" exact accesskey="h">
				<v-img style="max-width: 160px" alt="hablaqui Logo" :src="`https://cdn.hablaqui.cl/static/logo.png`"
					:lazy-src="`https://cdn.hablaqui.cl/static/logo.png`" contain />
			</a>
			<span class="hidden-sm-and-down text--secondary text-h7">
				¿Necesita ayuda?
				<a style="text-decoration: none" class="primary--text"
					href="https://api.whatsapp.com/message/RZPJ4H5ZTDNWB1?autoload=1&app_absent=0" target="_blank">
					Contáctenos
				</a>
			</span>
			<a style="text-decoration: none" class="hidden-md-and-up"
				href="https://api.whatsapp.com/message/RZPJ4H5ZTDNWB1?autoload=1&app_absent=0" target="_blank">
				<v-img style="max-width: 30px" alt="Ayuda" :src="`https://cdn.hablaqui.cl/static/help.png`"
					:lazy-src="`https://cdn.hablaqui.cl/static/help.png`" contain />
			</a>
		</div>
					...
					<v-stepper-items>
						<v-stepper-content step="1">
							<v-row>
								...
								<v-col cols="12" md="6">
									<v-menu ref="menu" v-model="bmenu" :close-on-content-click="false" transition="scale-transition"
										offset-y min-width="auto">
										<template #activator="{ on, attrs }">
											<v-text-field id="birthdate" v-model="form.birthDate" readonly filled outlined hide-details
												placeholder="(Requerido)" :rules="rulesTextField" dense v-bind="attrs" v-on="on"></v-text-field>
										</template>
										<v-date-picker v-model="form.birthDate" locale="es" :active-picker.sync="activePicker" :max="
											new Date(
								...
								<v-col cols="12" md="6">
									<v-select id="genre" v-model="form.gender" :items="[
										{ text: 'Hombre', value: 'male' },
										{ text: 'Mujer', value: 'female' },
										{ text: 'Transgénero', value: 'transgender' },
									]" filled outlined hide-details placeholder="(Requerido)" :rules="rulesTextField" dense></v-select>
								</v-col>
								<v-col cols="12" md="4">
									<div class="primary--text text-h7 mb-2 font-weight-regular">
										País
									</div>
									<v-autocomplete id="region" v-model="form.country" :items="countries" item-text="name" item-value="name"
										filled outlined hide-details dense placeholder="(Requerido)" :rules="rulesTextField" @change="
											(e) => {
								...
								<v-col cols="12" md="4">
									...
									<v-autocomplete id="region" v-model="form.region" :disabled="form.country !== 'Chile'" :items="regiones"
										filled outlined hide-details dense :placeholder="form.country !== 'Chile' ? '' : '(Requerido)'"
										:rules="rulesTextField"></v-autocomplete>
								</v-col>
								<v-col cols="12" md="4">
									<div class="primary--text text-h7 mb-2 font-weight-regular">
										Comuna
									</div>
									<v-autocomplete id="comuna" v-model="form.comuna" :disabled="!form.region || form.country !== 'Chile'"
										:items="comunas" filled outlined hide-details
										:placeholder="form.country !== 'Chile' ? '' : '(Requerido)'" :rules="rulesTextField"
								...
								<v-col cols="12" md="4">
									...
									<v-combobox id="timezone" v-model="form.timeZone" dense filled hide-details placeholder="(Requerido)"
										:rules="rulesTextField" :items="timezone" outlined :search-input.sync="zone">
										<template #no-data>
											<v-list-item>
												<v-list-item-content>
													<v-list-item-title>
								...
								<v-col cols="4" md="3" offset-md="1">
									...
									<v-autocomplete id="phonecode" v-model="form.phone.code" :items="countries" item-text="dialCode" filled
										outlined hide-details dense :rules="rulesTextField">
								...
								<v-col cols="8" md="4">
									...
									<v-text-field id="phoneNumber" v-model="form.phone.number" filled outlined type="number"
										:rules="rulesTextField" dense></v-text-field>
								</v-col>
								<v-col cols="12">
									<div class="primary--text text-h7 mb-2 font-weight-regular">
										Idiomas
									...
									<div class="d-flex">
										<v-checkbox v-model="form.languages" class="mx-2" value="spanish" filled label="Español" outlined
											hide-details dense></v-checkbox>
										<v-checkbox v-model="form.languages" class="mx-2" value="english" filled label="Ingles" outlined
											hide-details dense></v-checkbox>
									</div>
								</v-col>
								<v-col cols="12">
									...
									<v-textarea id="description-personal" v-model="form.personalDescription" outlined placeholder="Me agrada meditar, disfrutar de los pequeños milagros cotidianos, contemplar la naturaleza,
							agradecer, aprender de las personas, realizar actividades deportivas y reír." filled no-resize
										hint="Hable de usted más allá de lo profesional. Describa sus gustos y pasatiempos creando un ambiente acogedor."
										counter :rules="rulesDescriptionField"></v-textarea>
								</v-col>
								<v-col cols="12" md="6">
									<div class="primary--text text-h7 mb-2 font-weight-regular">
										Instagram
									</div>
									<v-text-field v-model="form.instagram" filled outlined dense type="text"
										placeholder="instagram.com/usuario"></v-text-field>
								</v-col>
								<v-col cols="12" md="6">
									<div class="primary--text text-h7 mb-2 font-weight-regular">
										LinkedIn
									</div>
									<v-text-field v-model="form.linkedin" placeholder="linkedin.com/in/usuario/" filled outlined dense
										type="text"></v-text-field>
								</v-col>
							</v-row>
							<div class="d-flex justify-end mt-4">
								<v-btn :loading="loadingStep" rounded color="primary" @click="saveStep(2)">
									Siguiente
								</v-btn>
							</div>
						</v-stepper-content>
						...
						<v-stepper-content step="2">
							<v-row>
								<v-col cols="12">
									...
									<v-textarea v-model="form.professionalDescription" no-resize filled outlined dense
										...
										hint="Describa su metodología de trabajo y cómo puedes ayudar a al consultante." counter
										:rules="rulesDescriptionField"></v-textarea>
								</v-col>
								<v-col cols="12" md="6">
									<div class="primary--text text-h7 mb-2 font-weight-regular">
										Formación
									...
									<v-dialog v-if="selectedFormation" v-model="dialogFormation" max-width="400" @click:outside="
										() => {
											...
											<v-card-text class="pt-8">
												<v-row>
													<v-col cols="12">
														...
														<v-select v-model="selectedFormation.formationType" filled outlined hide-details
															placeholder="Título" dense type="text" :items="[
																'Licenciatura',
																'Diplomado',
																'Magister',
																'Doctorado',
													...
													<v-col cols="12">
														...
														<v-text-field v-model="selectedFormation.description" filled outlined
															placeholder="Especialista Clinico" dense hide-details type="text"></v-text-field>
													</v-col>
													<v-col cols="12">
														<div class="primary--text body-1 pb-2">
															Institución
														</div>
														<v-text-field v-model="selectedFormation.intitucion" filled outlined
															placeholder="Universidad de Chile" dense hide-details type="text"></v-text-field>
													</v-col>
													<v-col cols="12">
														<div class="primary--text body-1 pb-2">Inicio</div>
														<v-text-field v-model="selectedFormation.start" filled outlined dense type="text"
															placeholder="2019" hide-details></v-text-field>
													</v-col>
													<v-col cols="12">
														<div class="primary--text body-1 pb-2">Termino</div>
														<v-text-field v-model="selectedFormation.end" filled outlined dense placeholder="2021"
								...
								<v-col cols="12" md="6">
									...
									<v-dialog v-if="selectedExperience" v-model="dialogExperience" max-width="400" @click:outside="
										() => {
											...
											<v-card-text class="pt-8">
												<v-row>
													<v-col cols="12">
														...
														<v-text-field v-model="selectedExperience.title" filled outlined
															placeholder="Especialista Clinico" dense hide-details type="text"></v-text-field>
													</v-col>
													<v-col cols="12">
														<div class="primary--text body-1 pb-2">
														...
														<v-text-field v-model="selectedExperience.place" filled outlined dense hide-details
															placeholder="Clinica Las Condes" type="text"></v-text-field>
													</v-col>
													<v-col cols="12">
														<v-checkbox v-model="selectedExperience.current" dense hide-details
															label="Actualmente trabajando aquí" @change="
																(e) => {
													...
													<v-col cols="12">
														<div class="primary--text body-1 pb-2">Inicio</div>
														<v-text-field v-model="selectedExperience.start" filled outlined dense hide-details
															placeholder="2019" type="text"></v-text-field>
													</v-col>
													<v-expand-transition>
														<v-col v-show="!hiddenInput" cols="12">
															...
															<v-text-field v-model="selectedExperience.end" filled outlined dense hide-details
																placeholder="2021" type="text"></v-text-field>
														</v-col>
													</v-expand-transition>
												</v-row>
											</v-card-text>
								...
								<v-col cols="12" md="6">
									...
									<v-select v-model="form.specialties" :loading="!specialties.length" filled outlined dense multiple
										hide-details type="text" class="pb-0" :items="specialties" @input="limiterSpecialities">
										<template #selection>
											<div></div>
										</template>
									</v-select>
								...
								<v-col cols="12" md="6" class="pt-0 hidden-md-and-up">
									<v-chip v-for="(item, i) in form.specialties" :key="i" outlined small class="ma-2" close
								...
								<v-col cols="12" md="6">
									...
									<v-select v-model="form.models" filled outlined dense multiple hide-details type="text" class="pb-0"
										:items="[
								...
								<v-col cols="12" md="6" class="pt-0">
									<v-chip v-for="(item, i) in form.models" :key="i" outlined small class="ma-2" close
							...
							<div class="d-flex justify-end mt-8">
								...
								<v-btn :loading="loadingStep" class="mx-2" rounded color="primary" @click="saveStep(3)">
									Siguiente
								</v-btn>
							</div>
						</v-stepper-content>
						...
						<v-stepper-content step="3">
							<v-row>
								<v-col cols="12">
									...
									<div>
										<v-text-field v-model="form.yearsExpSpecialist" filled outlined dense :rules="rulesTextField"
											type="text"></v-text-field>
									</div>
								</v-col>
								<v-col cols="12">
									...
									<div>
										<v-text-field v-model="form.yearsExpVideocalls" filled outlined :rules="rulesTextField" dense
											type="text"></v-text-field>
									</div>
								</v-col>
								<v-col cols="12">
									...
									<div>
										<v-text-field v-model="form.avgPatients" filled outlined dense :rules="rulesTextField"
											type="text"></v-text-field>
									</div>
								</v-col>
								<v-col cols="12">
									...
									<div>
										<v-select v-model="form.howFindOut" filled outlined dense type="text" :items="[
									...
									<div>
										<v-radio-group v-model="form.isExclusiveActivity" row>
											<v-radio v-for="n in [
												{ text: 'Si', value: true },
												{ text: 'No', value: false },
											]" :key="n.text" :label="n.text" :value="n.value"></v-radio>
										</v-radio-group>
									</div>
								</v-col>
								<v-col cols="12">
							...
							<div class="d-flex justify-end mt-4">
								...
								<v-btn :loading="loadingStep" class="mx-2" rounded color="primary" @click="
									() => {
							...
							<v-container fluid style="height: 70vh; max-width: 1200px">
								<v-row justify="center" align="center" style="height: 100%; overflow-y: auto">
									<v-col cols="12" class="text-center" style="color: #5c5c5c">
										<div>
											<v-img width="200" height="200" class="mx-auto" :src="`https://cdn.hablaqui.cl/static/balloon.png`"
												:lazy-src="`https://cdn.hablaqui.cl/static/balloon.png`"></v-img>
										</div>
										<div class="my-6 text--secondary body-1 mx-auto" style="max-width: 800px">
											Hemos recibido su postulación, en breve verificaremos sus
											datos.
										</div>
										<div>
											<v-btn depressed class="mx-2" color="primary" rounded :to="{ name: 'dashboard-perfil' }">
												Ir a mi cuenta
											</v-btn>
										</div>
									</v-col>
...
export default {
	...
	async mounted() {
		...
		const responseRecruitment = await this.$axios.$get(
	...
	methods: {
			...
			if (this.validationStep(step)) {
				if (this.form && this.form._id) {
					...
					const { data } = await this.$axios(
						'/recruitment/update?step=' + this.step,
						{
							method: 'put',
							data: this.form,
						}
		...
		newExperience() {
			if (this.indexSelected >= 0 && this.indexSelected !== null)
				this.form.experience[parseInt(this.indexSelected)] =
		...
		newFormation() {
			if (this.indexSelected >= 0 && this.indexSelected !== null)
				this.form.formation[parseInt(this.indexSelected)] =
