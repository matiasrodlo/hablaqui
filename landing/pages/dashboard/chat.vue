      ...
      <v-row v-show="!initLoading">
        <v-col cols="12" md="4" lg="3" class="px-0 px-md-4">
          <v-card
            ...
            :style="
              $vuetify.breakpoint.smAndDown ? '' : 'height: calc(100vh - 135px)'
            "
            class="py-4 mt-md-0"
          >
            ...
            <template
              v-if="
                $auth.$state.user && $auth.$state.user.role === 'specialist'
              "
            >
              <!-- sin consultantes -->
                ...
                >
                  ...
                  >
                    <v-list-item-avatar
                      ...
                      size="40"
                    >
                      <avatar :url="spec.avatar" :name="spec.name" size="40" />
                    </v-list-item-avatar>
...
export default {
  ...
  computed: {
    ...
    listUsers() {
      let filterArray = this.chats.filter(
        (item) => item.specialist && item.user
      )
    ...
    getMySpec() {
      if (
        this.$auth.$state.user &&
        this.$auth.$state.user.role === 'user' &&
        this.plan
      ) {
  ...
  methods: {
      ...
      if (this.$auth.$state.user.role === 'specialist') {
        ...
        if ('client' in this.$route.query) {
          this.setSelectedUser(
            this.clients.find(
              (client) => client._id === this.$route.query.client
            )
          )
          if ('client' in this.$route.query)
    ...
    async socketioCallback(data) {
      if (
        this.selected._id === data.specialistId ||
    ...
    async setSelectedUser(user) {
      ...
      await this.getChat({
        spec: this.$auth.$state.user.specialist,
        user: user._id,
      })
    ...
    hasMessage(spec) {
      const temp = {
        ...this.chats.find(
          (item) => item.specialist && item.specialist._id === spec._id
        ),
      }
      if (temp && temp.messages && temp.messages.length) {
        const hasMessage = temp.messages.some(
          (message) =>
            message &&
            !message.read &&
            message.sentBy !== this.$auth.$state.user._id
        )
