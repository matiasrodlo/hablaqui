<template>
  <div>
    ...
    <v-container
      fluid
      style="height: 100vh; max-width: 1200px; position: relative"
    >
      <card-onboarding
      ...
      <v-row justify="center" style="height: calc(100vh - 110px)">
        ...
        >
          ...
          >
            <div class="d-flex justify-center">
              <div>
                <div
                  class="body-2 text-center text--secondary font-weight-bold"
                >
                  Nº de Sesiones
                </div>
                <div
            ...
            <v-row justify="end" class="text-md-right pt-4">
              ...
              >
                <v-btn
                  text
                  depressed
                  @click="() => setFilter('sesion presencial')"
                >
        ...
        >
          ...
          <div
            class="body-2 text-center text--secondary font-weight-bold mt-16"
          >
            Próxima sesión
          </div>
          <div
...
export default {
  ...
  created() {
    ...
    this.resetForm()
    this.dialogAppointment = this.$route.query.dialog
  ...
  methods: {
    ...
    async initFetch() {
      if (this.plans) {
        this.plan =
          this.plans.sortedPlans.length > 0 ? this.plans.sortedPlans[0] : null
    ...
    setStepAddAppoinment() {
      if (this.validatenewCustomSession)
    ...
    async addAppointment({ date }) {
      if (this.$auth.user.role === 'user') {
        ...
        this.dialogSearchNow =
          !this.plan || (this.plan && !this.plan.specialist)
    ...
    async newSession(event) {
      ...
      if (payload.remainingSessions <= 0) {
        ...
        if (this.plans.sortedPlans.length > 0)
          this.plan = this.plans.sortedPlans[0]
      }
      this.loadingSession = false
