<template>
  <v-card
    :style="$vuetify.breakpoint.smAndDown ? 'width: 100%' : 'width: 400px'"
  >
    <div
      v-if="selected"
      class="d-flex"
    ...
    >
      <v-card-title
        style="flex: 0"
        class="primary--text d-flex justify-space-between"
      >
        Chat
      ...
      <template
        v-if="
          specFromChats.length ||
          ($auth.$state.user.role === 'user' && plan && !search)
        "
      >
        <v-card-text style="flex: 0" class="py-0">
          <v-subheader class="primary--text body-1 px-0"
            >Especialistas</v-subheader
          >
