<template>
  <v-row class="mb-16">
    <v-col cols="12" class="d-flex justify-space-between align-center">
      <div>
        <div
          class="font-weight-regular caption text-md-h6"
          style="color: #3c3c3b"
        >
...
export default {
  ...
  computed: {
    ...
    accountTypeError() {
      ...
      !this.$v.bankData.accountType.required &&
        errors.push('El tipo de cuenta es querido')
      return errors
    },
    /**
     * retorna error si no llena el campo
    ...
    emailError() {
      ...
      !this.$v.bankData.email.required &&
    ...
    hasChanges() {
      return (
        JSON.stringify(this.bankData) ===
        JSON.stringify(this.specialist.paymentMethod)
      )
    },
  },
  async mounted() {
    ...
    // listado de bancos
    let response = await fetch(`${this.$config.API_ABSOLUTE}/bancos.json`)
