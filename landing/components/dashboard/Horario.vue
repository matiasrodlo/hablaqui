...
export default {
  ...
  computed: {
    ...
    hasOverlay() {
      return this.items.some(
        (item) =>
          item.error && (item.error.start.length || item.error.end.length)
      )
    },
    /**
  ...
  methods: {
    ...
    validateInput(indexDay, indexInterval, value, type) {
      ...
      if (!result) {
        if (type === 'start') {
          this.items[indexDay].error.start = this.items[
            indexDay
          ].error.start.filter((el) => el !== indexInterval)
        }
        if (type === 'end') {
          this.items[indexDay].error.end = this.items[
            indexDay
          ].error.end.filter((el) => el !== indexInterval)
        }
      } else {
        if (type === 'start')
          this.items[indexDay].error.start.push(indexInterval)
