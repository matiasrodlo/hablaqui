  ...
  const validPayments = allSessions.flatMap((item) => {
    if (item.user)
      return item.plan.flatMap((plans) => {
        ...
        let amountDue = 0;
        let paymentPlanDate = moment(plans.datePayment).format("DD/MM/YYYY");

        let sessions = plans.session.map((session) => {
          let transDate =
            session.paymentDate &&
            moment(session.paymentDate, "MM/DD/YYYY").isValid()
              ? moment(session.paymentDate, "MM/DD/YYYY").format("DD/MM/YYYY")
              : session.requestDate && moment(session.requestDate).isValid()
              ? "Pendiente"
              : "Por cobrar";
          transDate = session.status === "pending" ? "Por realizar" : transDate;
          return {
            _id: session._id,
            datePayment: moment(session.date, "MM/DD/YYYY HH:mm").format(
              "DD/MM/YYYY"
            ),
  ...
  let session = sessions.plan
    .flatMap((plan) => {
      let maxSession = plan.session.map((session) =>
        moment(session.date, "MM/DD/YYYY HH:mm").format("YYYY/MM/DD HH:mm")
      );
