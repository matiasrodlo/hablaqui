    ...
    return item.plan.flatMap(plan => {
      return plan.session.map(session => {
        ...
        let requestDate = session.requestDate
          ? session.requestDate
          : 'Por cobrar'
        if (requestDate !== 'Por cobrar') { requestDate = dayjs(requestDate).format('YYYY/MM/DD HH:mm') }
        let paymentDate = session.requestDate
          ? session.requestDate
          : 'Por cobrar'
        if (paymentDate !== 'Por cobrar') { paymentDate = dayjs(paymentDate).format('YYYY/MM/DD HH:mm') }
        return {
          _id: session._id,
          date: session.date,
