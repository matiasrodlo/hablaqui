...
const evaluationController = {
  ...
  async getRating(req, res) {
    try {
      const { psychologist } = req.params;
      const { data, code } = await evaluationService.getRating(psychologist);
      return restResponse(data, code, res);
    } catch (e) {
  ...
  async getEvaluationsPsy(req, res) {
    try {
      const { user } = req;
      const { data, code } = await evaluationService.getEvaluationsPsy(user);
      return restResponse(data, code, res);
    } catch (e) {
  ...
  async getAllEvaluations(req, res) {
    try {
      ...
      const { data, code } = await evaluationService.getAllEvaluations(psy);
      return restResponse(data, code, res);
    } catch (e) {
