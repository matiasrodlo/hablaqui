...
const getSessions = async user => {
  ...
  if (user.role === 'psychologist') { return await Sessions.find({ psychologist: user.psychologist }) }
...
const register = async payload => {
  ...
  if (payload.role === 'psychologist') {
    if (await User.exists({ rut: payload.rut })) { return conflictResponse('Rut en uso') }
  }
  const newUser = {
    ...payload,
    isInvited: false,
  ...
  if (process.env.NODE_ENV === 'development') { logInfo(actionInfo(payload.email, `url: ${verifyurl}`)) }
...
const sendPasswordRecover = async email => {
  ...
  if (process.env.NODE_ENV === 'development') { logInfo(actionInfo(email, `url: ${recoveryUrl}`)) } else logInfo(actionInfo(email, 'solicito una recuperación de contraseña'))
