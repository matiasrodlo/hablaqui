...
const startConversation = async (psychologistId, user) => { // función que recibe el id del psicologo y el usuario, función que inicializa una conversación
	const hasChats = await Chat.findOne({ // busca un chat en la base de datos
	...
	if (!hasChats) { // si no existe un chat con el psicologo y el usuario
		const newChat = await Chat.create({ // crea un nuevo chat
...
const getMessages = async (user, psy) => { // función para obtener los mensajes de un chat, user y psy son ObjectID del usuario y el psicologo
	let messages = await Chat.findOne({ // busca un chat en la base de datos
	...
	if (!messages) // si no existe un chat con el psicologo y el usuario
		messages = await Chat.create({ // crea un nuevo chat
			user: user, // con el id del usuario
			psychologist: psy, // y el id del psicologo
		});

	return okResponse('Mensajes conseguidos', { // retorna un mensaje de éxito y los mensajes
		messages,
	});
};

const getChats = async user => { // función para obtener los chats de un usuario
	if (user.role == 'psychologist') { // si el usuario es un psicologo
		logInfo(`El psicologo ${user.email} ha conseguido sus chats`); // imprime en la consola el email del psicologo
		return okResponse('Chats conseguidos', { // retorna un mensaje de éxito y los chats
			chats: await Chat.find({ // busca los chats en la base de datos
	...
	if (user.role == 'user') { // si el usuario es un usuario
		logInfo(`El usuario ${user.email} ha conseguido sus chats`); // imprime en la consola el email del usuario
		return okResponse('Chat conseguidos', { // retorna un mensaje de éxito y los chats
			chats: await Chat.find({ user: user._id }).populate( // busca los chats en la base de datos y hace que devuelva del documento de chat el usuario y el psicologo
				'user psychologist'
			),
		});
	}
	return conflictResponse( // si el usuario no es ni un usuario ni un psicologo retorna un mensaje de error
		'Ha ocurrido un error intentando recuperar tus chats'
	);
};

export const sendMessage = async (user, content, userId, psychologistId) => { // función para enviar un mensaje, recibe el usuario, el contenido del mensaje, el id del usuario y el id del psicologo
	const newMessage = { // crea un nuevo mensaje
		sentBy: user._id, // con el id del usuario que lo envía
		message: content, // y el contenido del mensaje
	};

	const updatedChat = await Chat.findOneAndUpdate( // busca un chat en la base de datos
		{
		...
		{
			$push: {// añade un nuevo mensaje al chat
	...
	await emailChatNotification( // envía un email de notificación al psicologo
	...
	analytics.track({ // envía un evento a segment
...
const emailChatNotification = async (data, type) => { // función para enviar un email de notificación, recibe los datos del mensaje y el tipo de notificación
	const payload = { // crea un objeto con los datos del mensaje
	...
	await Email.updateOne( // busca un email en la base de datos
		{
...
const createReport = async ( // función para crear un reporte, recibe el id del usuario, el id del psicologo, el id del chat, el contenido del reporte y el motivo del reporte
	...
	const newReport = { // crea un nuevo reporte
		reportedBy: user._id, // con el id del usuario que lo reporta
		reportType, // con el tipo de reporte
		issue, // con el motivo del reporte
	};

	const updatedChat = await Chat.findOneAndUpdate( // busca un chat en la base de datos para actualizarlo
		{
			user: userId, // que tenga como usuario al usuario que se recibe como parámetro siendo de tipo ObjectId
			psychologist: psychologistId, // y que tenga como psicologo al psicologo que se recibe como parámetro siendo de tipo ObjectId
		},
		{
			$push: { // añade un nuevo reporte al chat
	...
	logInfo( // registra un log de información
...
const readMessage = async (user, chatId) => {
	...
	await Chat.updateOne( // busca un chat en la base de datos para actualizarlo
...
const chatService = { // crea un objeto con las funciones del servicio
	startConversation,
	getMessages,
	getChats,
	sendMessage,
