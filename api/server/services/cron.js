...
function isSchedulableEmail(date) {
   ...
   * @returns
   */
  return moment().add(3, "days").isAfter(date);
}
  ...
  users.forEach(async (user) => {
    ...
    sessions.forEach(async (item) => {
      ...
      await Sessions.updateOne(
        ...
        },
        {
          $set: {
            ...
            evaluationNotifcation: successSessions >= 3 ? true : false,
          },
        }
      );
      if (!item.evaluationNotifcation && successSessions === 3) {
        ...
        await mailServicePsy.sendEnabledEvaluation(user, item.psychologist);
      }
    });
  });
}
  ...
  async scheduleChatEmails(token) {
    ...
    dontReadMess.forEach(async (mess) => {
      ...
      if (mess.lastMessageSendBy === "user")
        await mailServiceRemider.sendChatNotificationToPsy(user, psy, batch);
      else if (mess.lastMessageSendBy === "psychologist")
        await mailServiceRemider.sendChatNotificationToUser(user, psy, batch);
    });
      ...
      pendingEmails.forEach(async (emailInfo) => {
        ...
        if (isSchedulableEmail(sessionDate)) {
          ...
          try {
            ...
            const updatePayload = generatePayload(sessionDate, batch);
  ...
  async limitToPayPlan(token) {
    ...
    sessions.forEach((item) => {
      ...
      plans.forEach(async (plan) => {
        if (moment().isSameOrAfter(moment(plan.createdAt).add(3, "hours"))) {
          ...
          await mailServicePsy.sendPaymentFailed(item.user, item.psychologist);
        }
      });
    });
