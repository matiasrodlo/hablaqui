...
const cronService = {
  ...
  async sessionStatus(token) {
    ...
    const toUpdateUpnext = [];
    const toUpdateSuccess = [];
        ...
        await item.plan.map(async (plan) => {
          await plan.session.map(async (session) => {
            ...
            if (
              ...
              moment(date)
                .subtract(psyInfo.preferences.minimumRescheduleSession, "hours")
                .isBefore(moment()) &&
              ...
              (session.status === "upnext" || session.status === "pending") &&
              moment().isAfter(date)
            ) {
    ...
    if (toUpdateUpnext.length > 1) {
        ...
        await Promise.allSettled(
          toUpdateUpnext.forEach(async (item) => {
            await Sessions.findOneAndUpdate(
              ...
              {
                $set: {
                  "plan.$[].session.$[element].status": item.status,
                },
              },
              {
      ...
      try {
        ...
        await Sessions.findOneAndUpdate(
          ...
          {
            $set: {
              "plan.$[].session.$[element].status": toUpdateUpnext[0].status,
            },
          },
          {
    ...
    if (toUpdateSuccess.length > 1) {
      ...
      try {
        await Sessions.findOneAndUpdate(
          ...
          {
            $set: {
              "plan.$[].session.$[element].status": toUpdateSuccess[0].status,
            },
          },
          {
            arrayFilters: [{ "element._id": toUpdateSuccess[0].id }],
            new: true,
          }
        );
      } catch (error) {
