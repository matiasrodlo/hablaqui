...
const addRating = async (user, newRating, comment, psychologist) => {
  ...
  if (user.psychologist != psychologist) { return conflictResponse('Este no es tu psicologo') }
...
const getRating = async psychologist => {
  ...
  if (!foundPsychologist.ratings || foundPsychologist.ratings.length == 0) { return okResponse('El psicologo no tiene evaluaciones aun.') }
...
const getEvaluationsPsy = async user => {
  if (user.role !== 'psychologist') { return conflictResponse('No eres psicólogo') }
...
const addEvaluation = async (user, psyId, payload) => {
  ...
  if (countSessions < 3) { return conflictResponse('No puede escribir un comentario') }
