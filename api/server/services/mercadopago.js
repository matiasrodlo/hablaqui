  ...
  if (
    ...
    analytics.track({
      ...
      properties: {
        ...
        products: planData,
        order_id:
          response.psyPlans[response.psyPlans.length - 1]._id.toString(),
        total: 0,
      },
    });
  }
...
const psychologistPay = async (params, query) => {
  ...
  let expirationDate;
  if (period === "anual") {
    expirationDate = moment().add({ months: 12 }).format();
  }
  if (period === "mensual") {
    expirationDate = moment().add({ months: 1 }).format();
  }
