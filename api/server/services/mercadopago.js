...
const createPsychologistPreference = async body => {
  ...
  let preference = {}
  if (body.plan === 'premium') { preference.init_point = await setPlanPremium(body, isPsychologist, id) } else {
...
const setPlanFree = async (id, isPsychologist) => {
  ...
  if (!response) { return conflictResponse('No se encontró el postulante o psicologo') }
