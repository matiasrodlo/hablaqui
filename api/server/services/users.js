...
const usersService = {
  ...
  async updatePassword (user, oldPassword, newPassword) {
    ...
    if (!isEqual) { return conflictResponse('la contraseña anterior no es correcta') } else return await this.changeActualPassword(foundUser, newPassword)
  },
  async passwordRecovery (user, newPassword) {
    ...
    if (isEqual) { return conflictResponse('no puede ser la misma contraseña') } else return await this.changeActualPassword(foundUser, newPassword)
  },
  ...
  async uploadAvatar ({
    ...
    if (!avatar && !avatarThumbnail) { return conflictResponse('Ha ocurrido un error inesperado') }
  ...
  async registerUser (user, body) {
    if (user.role !== 'psychologist') { return conflictResponse('Usuario activo no es psicologo') }
    if (await User.exists({ email: body.email })) { return conflictResponse('Correo electronico en uso') }
