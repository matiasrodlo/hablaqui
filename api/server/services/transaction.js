...
const createPaymentsRequest = async user => {
  if (user.role === 'user') { return conflictResponse('No estas autorizado para esta operacion') }
...
const generateTransaction = async (user, total, session, idPsy) => {
  ...
  if (session.length === 0) { return conflictResponse('No hay sesiones para pagar') }
