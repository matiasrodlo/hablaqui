...
const getNextSessions = async () => {
  ...
  const plans = sessions
    .flatMap((s) => {
      ...
      const planActived =
        plan.payment === "success" && moment(plan.expiration).isAfter(moment());
...
const fixSpecialities = async () => {
  ...
  for (let j = 0; j < psychologists.length; j++) {
    ...
    for (let i = 0; i < psychologists[j].specialties.length; i++) {
      const index = arrayAppointments.indexOf(psychologists[j].specialties[i]);
      if (index !== -1) arraySpecialities.push(psychologists[j].specialties[i]);
    }
  ...
  for (let psy in psychologists) {
    ...
    session = session.flatMap((item) =>
      item.sessions.flatMap((s) => {
        return {
          date: moment(s.date, "MM/DD/YYYY HH:mm").format("DD/MM/YYYY HH:mm"),
          _id: s._id,
          status: s.status,
