...
const updatePaymentMethod = async (user, payload) => {
  if (user.role !== 'psychologist') { return conflictResponse('No eres un psicologo.') } else {
  ...
  if (user.psychologist) {
    ...
    try {
      ...
      if (psy.sessionPrices.video !== profile.sessionPrices.video) {
        ...
        ) { profile.sessionPrices = psy.sessionPrices } else profile.stampSetPrices = dayjs().format()
      }
...
const changeToInmediateAttention = async psy => {
  ...
  if (psychologist.inmediateAttention.activated) {
    ...
    if (sessions.length !== 0) { return conflictResponse('Tiene sesiones próximas') }
