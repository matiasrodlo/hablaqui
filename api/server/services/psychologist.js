...
const criteriaNumberSpecialties = (psy, payload, pointsPerCriterion) => {
  ...
  for (let j = 0; j < numberOfSpecialities; j++) {
    if (psy.specialties[j] === payload.themes[j]) points += pointsPerCriterion;
...
const maximumAvailability = (payload, pointsPerCriterion) => {
  ...
  if (payload.schedule == "afternoon") maximum = (23 - 16) * pointsPerCriterion;
  return maximum;
};
...
const ponderationMatch = async (matchedList, payload) => {
  ...
  let newMatchedList = await Promise.all(
    matchedList.map(async (psy) => {
      ...
      points +=
        priceCriterion(psy, payload, pointsPerCriterion) * weighted[criteria];
      criteria++;
...
const match = async (body) => {
  ...
  matchedPsychologists = await ponderationMatch(matchedPsychologists, payload);
...
const updatePaymentMethod = async (user, payload) => {
  if (user.role !== "psychologist")
    ...
    const newPaymentMethod = {
      ...
      accountNumber:
        payload.accountNumber || foundPsychologist.paymentMethod.accountNumber,
        ...
        analytics.identify({
          ...
          traits: {
            ...
            hasProfessionalDescription:
              updated.professionalDescription == "" ? false : true,
  ...
  if (psychologist.inmediateAttention.activated) {
    ...
    sessions = sessions.filter((session) => {
      ...
      return (
        ...
        moment(date).add(50, "minutes").isAfter(moment(now))
      );
    });
    ...
    psychologist = await Psychologist.findOneAndUpdate(
      { _id: psy },
      {
        $set: {
          inmediateAttention: {
            activated: true,
            expiration: moment(now).add(1, "hour").format(),
          },
        },
      },
      { new: true }
