...
const setSession = (role, sessions) => {
  return sessions.flatMap((item) => {
    ...
    if (role === "psychologist") {
      ...
      lastName = item.psychologist.lastName ? item.psychologist.lastName : "";
    }
...
const createPlan = async ({ payload }) => {
  ...
  if (payload.paymentPeriod == "Pago semanal") {
    sessionQuantity = 1;
    expirationDate = moment().add({ weeks: 1 }).toISOString();
  }
  ...
  if (payload.paymentPeriod == "Pago cada tres meses") {
    sessionQuantity = 12;
    expirationDate = moment().add({ months: 3 }).toISOString();
  }
...
const paymentsInfo = async (user) => {
  ...
  const { psyPlans } = await Psychologist.findById(user.psychologist);
