...
const newCoupon = async (user, payload) => {
  ...
  if (user.role !== 'superuser') { return conflictResponse('No tienes poder aqui.') }
  if (await Coupon.exists({ code: payload.code })) { return conflictResponse('Ya hay un cupon con ese codigo') }
...
const checkCoupon = async (code, user) => {
  ...
  if (!foundCoupon) { return conflictResponse('No se ha encontrado un cupon con ese codigo') }
  if (dayjs().isAfter(foundCoupon.expiration)) { return conflictResponse('Este cupon ya ha expirado') }
  if (foundCoupon.discountType === 'static' && foundCoupon.discount === 0) { return conflictResponse('Cupón con saldo 0') }
  if (foundCoupon.restrictions) {
    if (foundCoupon.restrictions.firstTimeOnly && user.hasPaid) { return conflictResponse('Este usuario ya ha comprado alguna vez') }
    if (
