...
const createPlan = async ({ payload }) => {
  ...
  if (
    ...
    moment().isAfter(
      moment(date, "MM/DD/YYYY HH:mm").subtract(minimumNewSession, "hours")
    )
  ) {
  ...
  if (payload.paymentPeriod == "Pago mensual") {
    sessionQuantity = 4;
    expirationDate = moment().add({ months: 2 }).format();
  }
  if (payload.paymentPeriod == "Pago trimestral") {
    sessionQuantity = 12;
    expirationDate = moment().add({ months: 6 }).format();
  }
...
const createSession = async (userLogged, id, idPlan, payload) => {
  ...
  if (payload.remainingSessions === 0) {
    ...
    const expiration = moment(session.lastSession).add(50, "minutes").format();
...
const customNewSession = async (user, payload) => {
  try {
    ...
    for (let i = 0; i < hours; i++) {
      const date = moment(payload.date, "MM/DD/YYYY HH:mm").add(i, "hours");
      const newSession = {
...
const getFormattedSessionsForMatch = async (idPsychologist) => {
  ...
  const hours = Array.from(Array(24), (_, x) =>
    moment().hour(x).minute(0).format("HH:mm")
  );
  ...
  const daySessions = psySessions
    ...
    .filter((date) => moment(date, "MM/DD/YYYY HH:mm").isSameOrAfter(moment()));
...
const formattedSessionsAll = async (ids) => {
  ...
  allSessions = psychologist.map((item) => ({
    ...
    sessions: setDaySessions(
      allSessions.filter(
        (element) => element.psychologist._id.toString() === item._id.toString()
      )
    ),
  }));
        ...
        return {
          ...
          available: hours.filter((hour) => {
            return (
              ...
              !item.sessions.some(
                (date) =>
                  moment(date, "MM/DD/YYYY HH:mm").format("L") === temporal &&
                  hour === moment(date, "MM/DD/YYYY HH:mm").format("HH:mm")
              )
            );
          }),
...
const reschedule = async (userLogged, sessionsId, id, newDate) => {
  ...
  const { minimumRescheduleSession } = currentSession.psychologist.preferences;
