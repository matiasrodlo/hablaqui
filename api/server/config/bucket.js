...
const getPublicUrl = filename => { // Se define una función a la cual se le entrega un nonbre de archivo, y devuelve la URL pública.
  return `https://storage.googleapis.com/${bucketName}/general/${filename}`
}

// public URL for avatars (in full resolution)
const getPublicUrlAvatar = filename => { // Más especifico para las imágenes de perfil.
  return `https://cdn.hablaqui.cl/profile-pictures/${filename}`
}

// public URL for thumnail avatars (in thumbnail resolution)
const getPublicUrlAvatarThumb = filename => { // Especifico para las imágenes de perfil en miniatura para psycologos.
