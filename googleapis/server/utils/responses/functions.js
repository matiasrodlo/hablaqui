...
export const restResponse = (data, code, res) =>
	res.status(code).json({ ...data, status: true, apiVersion: 'v1', baseUrl: 'localhost:3000' });
...
export const errorResponse = (e, res) =>
	res.status(400).json({ status: false, error: e.message, description: e.description });

export const createdResponse = (message, data = {}) => ({ code: 201, data: { message, ...data } });

export const okResponse = (message, data = {}) => ({ code: 200, data: { message, ...data } });
